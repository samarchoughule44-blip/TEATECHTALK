...
export default async function ProfilePage() {
  ...
  if (roll_no && roll_no !== "N/A") {
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/profile?participantCode=${encodeURIComponent(roll_no)}`,
        { cache: 'no-store' }
      );
      if (res.ok) {
  ...
  type ActivityLog =
    ...
    | { kind: "room"; id: string; completedAt: string | Date; roomCode: string; finalScore: number; typingScore: number; quizScore: number; wpm: number | null; accuracy: number | null };

  const activityLog: ActivityLog[] = [
    ...typingResults.slice(0, 10).map((r) => ({ kind: "typing" as const, ...r })),
    ...roomActivities.slice(0, 10).map((r) => ({ kind: "room" as const, ...r })),
  ].sort((a, b) => {
    const aDate = new Date(a.kind === "typing" ? a.created_at : a.completedAt);
    const bDate = new Date(b.kind === "typing" ? b.created_at : b.completedAt);
                    ...
                    <>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-500 text-sm font-medium">
                          {formatDate(typeof entry.completedAt === 'string' ? entry.completedAt : entry.completedAt.toISOString())} · Typing + Quiz
                        </p>
